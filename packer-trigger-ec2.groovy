...
node {
    stage('Pull Repo') {
        git 'https://github.com/ahmetbekan/packer-jenkins.git'
    }
    ...
    withCredentials([usernamePassword(credentialsId: 'aws_jenkins_key', passwordVariable: 'AWS_SECRET_ACCESS_KEY', usernameVariable: 'AWS_ACCESS_KEY_ID')]) {
        withEnv(["AWS_REGION=${aws_region_var}", "PACKER_AMI_NAME=${ami_name}"]) {
            ...
            stage('Build EC2 Instance') {
                build wait: false, job: 'terraform-packer-ec2', parameters: [booleanParam(name: 'terraform_apply', value: true), booleanParam(name: 'terraform_destroy', value: false), string(name: 'environment', value: "${params.environment}"), string(name: 'ami_name', value: "${ami_name}")]
            }
        }     
    }
}
